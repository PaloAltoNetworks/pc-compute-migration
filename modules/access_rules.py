...
def migrate(dst_session, src_session_list, options, single_mode, logger, prefix=''):
    ...
    generic_pull_put_migrate.g_migrate(dst_session, src_session_list, 'Access Rules - Admission', '/api/v1/policies/admission', 'name', 'rules', single_mode, logger, skip='owner', skip_value='system', col_dep=False, prefix=prefix)
    ...
    for src_session in tqdm(src_session_list, desc='Processing Consoles/Projects', leave=False, initial=1):
        ...
        if src_entities:
            for ent in src_entities:
                if create_name(single_mode, src_session.tenant,ent['name'], prefix) not in [dst_ent['name'] for dst_ent in dst_entities]:
                    ent['name'] = create_name(single_mode, src_session.tenant,ent['name'], prefix)
